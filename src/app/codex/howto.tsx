      ...
      <Section id="cx-h-start" title="Start here">
        <Items
          items={[
            ...
            { name: 'Chronicles and feats', desc: 'Chronicles keep your record with each faction, your Legends medals, fifteen feats to earn (a flawless victory, a colossus brought down, walls taken or held, every Legend in gold, a week of Daily Battles, a campaign won with each faction…) and every tale and saga Claude has told you. They live in this browser.' },
            { name: 'Legends', desc: 'Six set battles, each built around one idea: the bell charge, the silent charge, the downwind run, light against dark, the Veil, and the Choir’s mirrors. Win for bronze; win losing fewer soldiers for silver and gold. Your best medals are kept. The Daily Battle above them is new each day: one matchup, one field and two armies of equal worth, the same for everyone who plays that day.' },
      ...
      <Section id="cx-h-campaign" title="The campaign">
        <Items
          items={[
            ...
            { name: 'Dilemmas', desc: 'Now and then the world puts a choice to you: pilgrims at the border, deserters, a cracked bell. Each choice shows its price before you make it.' },
            ...
            { name: 'Claude as counsel', desc: 'Played on claude.ai, Settings (or the new-campaign screen) can let the AI factions take Claude’s advice: every other Toll each asks Claude, in character, whether to change its wars, treaties or plans, envoys decide your closer proposals and answer in their own words, and now and then a dilemma is written for the moment (the game still sets what each choice gives and costs). In battles, the enemy general reads the field while you deploy, chooses to attack, hold or lie in ambush, and speaks; during the fight it thinks again when the lines meet, the battle turns or a colossus falls, and you hear its orders. A general who falls gives no more. In a battle you watch, both generals take Claude’s counsel. Your own council, in the Faction panel, suggests what to do this Toll when you ask; while you deploy, ✦ Counsel asks for three tips for the battle ahead, and in the pause menu your adviser reads where every unit stands and names the orders that matter now; and after a battle, ✦ Tell the tale has Claude tell it as your people’s chronicler would, from its turning points. Choices made on its advice are marked ✦ Claude in the chronicle.' },
          ]}
        />
        <p class="cx-note">
