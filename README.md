...
- **Legends.** Six set battles built around the design's signature moments (the bell charge, the silent charge, the downwind run, the colossus duel, light wars, the Choir's mirrors), each with its story and lesson. Win for bronze; win losing fewer soldiers for silver and gold. Above them, the **Daily Battle**: a new matchup, field and pair of equal armies each day, the same for everyone that day, with the run of days won kept.
- **Chronicles.** Your record with each faction (battles and campaigns won, the fastest victory), your Legends medals, fifteen feats to earn, and every tale and saga Claude has told you, kept in the browser.
- **Battle reports** list the battle's turning points (the lines meeting, routs, colossi and characters falling or using their powers, big charges, the generals' orders). **Save a picture** of the field from the battle menu, or with P.
...
- **Sound.** Procedural audio with no samples: the Choir sing their charges over glass harmonica, the Hush are near-silent until one shriek, the Vesperate ring bells and sound brass, the Drift throat-sing under whistling arrows and howling kites. Units you can't see make no sound. War drums in your faction's voice follow the fighting: silent in the lulls, gathering as the lines lock.
...
- **Claude as counsel (the design's "Jev").** Played as an artifact on claude.ai, Settings can let the campaign's AI factions take Claude's advice: every other Toll each faction asks Claude, in character and all at once, whether to change its wars, treaties or plans, and envoys decide your closer proposals in character (the game values every deal first; a clear insult or a generous offer needs no envoy) and answer in their own words. Now and then a dilemma is written for the moment from the campaign's recent events: Claude writes the words and picks each choice's boon and cost from the game's menu, and the game sets the amounts. The scripted AI always decides the details and takes over whenever Claude is slow, unavailable or declined, and the same limits apply (no wars of choice before Toll 14). In battles, while you deploy, the enemy general reads the field in words and picks a plan (attack, hold or ambush) and says a few words; the plan becomes part of the battle's setup, so the battle and its replay follow it exactly, and a late answer is dropped. During the fight it thinks again at the turning points (the lines meet, the battle turns, a colossus or general falls, nobody attacks), up to four times, and switches between pressing and holding; the change goes in as a recorded order, so replays follow it and show its words again. (The design's every-2-seconds Jev general would need answers far faster than a chat model gives.) In a battle you watch, both generals take Claude's counsel. At the end of a campaign, Claude's chronicler writes the saga of your war from its annals. `src/campaign/jev.ts` also takes any other advisor, such as an HTTP endpoint on a server you run.
- **Claude on request.** Whatever the AI setting, the ✦ buttons ask Claude only when pressed: your council in the campaign's Faction panel suggests three things to do this Toll; ✦ Counsel, while you deploy, gives three tips for the battle ahead; your adviser, in the battle menu, reads where every unit stands and names the orders that matter now; ✦ Tell the tale has a battle told as your people's chronicler would (campaign tales join the chronicle and the saga); and ✦ Invent a battle, in Custom Battle, makes up a battle with a name, a story and both armies, about whatever you ask for, checked against the rules before it is used.
...
Battle: left-click selects, drag a box to select many, right-click moves or attacks, right-drag lays out a line and its facing. Wheel zooms, WASD or arrows pan, and the minimap (larger screens) moves the view. Enter starts the battle from deployment. Space pauses, Esc opens the menu, R run, F fire at will, H halt, M melee, 1–3 abilities, +/− speed, Tab next unit, P saves a picture.
