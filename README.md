...
- **Claude as counsel (the design's "Jev").** Played as an artifact on claude.ai, Settings can let the campaign's AI factions take Claude's advice: every other Toll each faction asks Claude, in character and all at once, whether to change its wars, treaties or plans, and envoys answer your proposals in their own words. The scripted AI always decides the details and takes over whenever Claude is slow, unavailable or declined, and the same limits apply (no wars of choice before Toll 14). `src/campaign/jev.ts` also takes any other advisor, such as an HTTP endpoint on a server you run.
